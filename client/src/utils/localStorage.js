...
//If there is no value associated with this key, localStorage.getItem('saved_books') will return null. 
  const savedBookIds = localStorage.getItem('saved_books')
// ? - ternary operator to check if the value returned from localStorage.getItem('saved_books') is truthy (i.e., not null, undefined, false, 0, or an empty string). 
//If the value is truthy, the statement calls the JSON.parse() method to convert the string value to an object or array, and assigns the result to the savedBookIds variable.
//If the value is falsy, the statement assigns null to the savedBookIds variable.
    ? JSON.parse(localStorage.getItem('saved_books'))
    : null;
